...
int clean_and_repair_broken_html_test(tidy_workspace_t* w)
{
  TIDY_HTML_LOG

  const char* html = "<p>Hello<p>World";
  char* result = tidy_clean_and_repair(w, html);
  printf("-> '%s'\n", result);
  if(strcmp(result, "<p>Hello</p><p>World</p>") != 0) {
    tidy_free(result);
...
#define max_tests 3
int (*test[max_tests])() = {clean_and_repair_broken_html_test, clean_and_repair_html_string_test, clean_and_repair_html_empty_test};
