...
void clean_and_repair(tidy_workspace_t* workspace, const char* html, vec_eterm_t* term_array)
{
  char* result = tidy_clean_and_repair(workspace, html);
...
{
  ETERM* response = NULL;
  ETERM* pattern = erl_format("{clean_and_repair, Html, Options}");
  ...
  if (erl_match(pattern, emsg->msg)) {
    ...
    // clean_and_repair(workspace, html, &term_array);
    // ETERM* term_list = eterm_vec_to_list(term_array);
    // response = erl_format("{parse, ~w}", term_list);

    char* result = tidy_clean_and_repair(workspace, html);
    response = erl_format("{clean_and_repair, ~w}", erl_mk_binary(result, strlen(result)));
    tidy_free(result);
